...
public class PaellaDeliveryActivity extends AppCompatActivity implements OnNavigationReadyCallback, NavigationListener,
  ...
  public void onResponse(Call<DirectionsResponse> call, Response<DirectionsResponse> response) {
    if (validRouteResponse(response)) {
      ...
      map = navigationView.retrieveMapboxMap();
  ...
  private void animateCameraBbox(LatLngBounds bounds, int animationTime, int[] padding) {
    CameraPosition position = navigationView.retrieveMapboxMap().getCameraForLatLngBounds(bounds, padding);
    navigationView.retrieveMapboxMap().animateCamera(CameraUpdateFactory.newCameraPosition(position), animationTime);
  }
